...
export default function SignupPage() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    window.alert("데모라서 실제 회원가입은 동작하지 않습니다.");
  };
  ...
  return (
    <section className="mx-auto flex max-w-md flex-col gap-4 rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-lg shadow-slate-200">
      <h1 className="text-2xl font-semibold text-[#052A49]">회원가입</h1>
      <p className="text-sm text-[var(--text-muted)]">HK에서 단기임대 경험을 시작하세요.</p>
      ...
      <p className="text-xs text-[var(--text-muted)]">
        이미 계정이 있으세요?
        <Link className="ml-1 font-semibold text-[#052A49]" href="/login">
          로그인
        </Link>
      </p>
    </section>
